...
class Grid:
    ...
    def add(self, point):
        ...
        if not (0 <= column <= self.__max_size_bin) and \
           not (0 <= row <= self.__max_size_bin):
            return None
...
class GridTest(TestCase):
    ...
    def test_nearby_neighborhood(self):
        self.grid = Grid(3, 10)
        points =[Point(1,1), Point(1,4), Point(1,7),
                 Point(4,1), Point(4,4), Point(4,7),
                 Point(7,1), Point(7,4), Point(7,7)] 
        for p in points:
            self.grid.add(p)
